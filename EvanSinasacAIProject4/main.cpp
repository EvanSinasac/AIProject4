...
// Description:
//			Use Fuzzy Logic to have vehicles that react to obstacles and move out of the way of them
...
int main(int argv, char** argc)
{
	...
	//ss << SOLUTION_DIR << "common\\assets\\textures\\resourceMap7.bmp";
...
//					// 2 = directional light
	...
	while (!glfwWindowShouldClose(pWindow))
	{
		...
		{
			::vec_pFLEntities[index]->Update(deltaTime);
		}
		...
		{
			matModel = glm::mat4(1.0f);
			DrawObject(::vec_pFLEntities[index]->m_Mesh,
				matModel,
				matModel_Location,
				matModelInverseTranspose_Location,
				program,
			...
			for (unsigned int indexB = 0; indexB != 5; indexB++)
			{
				matModel = glm::mat4(1.0f);
				DrawObject(::vec_pFLEntities[index]->m_Sensors[indexB]->m_Mesh,
					matModel,
					matModel_Location,
					matModelInverseTranspose_Location,
					program,
...
void loadAIModels()
{
	...
	modelLocations.push_back("Low Poly Tank Model 3D model_XYZ_N_RGBA_UV.ply");
}
	...
	for (unsigned long x = 0; x < imageWidth; x++) {
		for (unsigned long y = 0; y < imageHeight; y++) {
			...
			else if (c == '_')		// not traversable
			{
				...
				::obstaclePositions.push_back(tempMesh->positionXYZ);
			}
...
void MakeFLEntities()
{
	for (unsigned int index = 0; index != spawnPoints.size(); index++)
	{
		//cFuzzyLogicEntity* tempEntity = new cFuzzyLogicEntity(spawnPoints[index], glm::vec3(0.0f, 1.0f, 0.0f));
		cFuzzyLogicEntity* tempEntity = new cFuzzyLogicEntity(spawnPoints[index], 
			glm::vec3(0.0f, 1.0f, 0.0f),
			glm::vec4(index / 10.0f, ((int)index % 2) / 2.0f, ((int)index % 3) / 3.0f, 1.0f));
		vec_pFLEntities.push_back(tempEntity);
	}
}